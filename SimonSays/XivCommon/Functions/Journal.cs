...
public class Journal {
    ...
    private readonly OpenQuestDelegate? openQuest;

    internal Journal(ISigScanner scanner) {
        if (scanner.TryScanText(Signatures.OpenQuest, out var openQuestPtr, "Journal (open quest)")) {
            this.openQuest = Marshal.GetDelegateForFunctionPointer<OpenQuestDelegate>(openQuestPtr);
        }
    }
    ...
    public unsafe void OpenQuest(uint questId) {
        if (this.openQuest == null) {
            throw new InvalidOperationException("Could not find signature for open quest function");
        }

        var agent = (IntPtr) Framework.Instance()->GetUIModule()->GetAgentModule()->GetAgentByInternalId(AgentId.QuestJournal);

        this.openQuest(agent, (int) (questId & 0xFFFF), 1, 0, 1);
    }
